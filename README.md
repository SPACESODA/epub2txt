...
* **一括処理**: 複数のファイルや最上位フォルダを一度に変換します (再帰的ではありません)。
* **整形**: 段落間に空行を追加します。
...
* **出力処理**: `-o` が新しいフォルダを指している場合は作成されます。`-o` は単一入力専用です。
...
* **出力場所**: `-o` なしの場合、各 `.epub` は元のファイルの隣に `.txt` として出力されます。`-o` は複数入力では使用できません。
* **フォルダスキャン**: 1階層のみスキャンし、サブフォルダはスキップされます。
* **OPF フォールバック**: EPUB の spine が空の場合、すべての HTML/XHTML ファイルをアーカイブ順に処理します。
* **エンコーディング**: 出力ファイルは UTF-8 エンコーディングに強制されます。
